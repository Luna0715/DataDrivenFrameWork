...
class ParseExcel(object):
    ...
    def getCellOfValue(self,sheet,coordinate = None,rowNo = None,colsNo = None):
        #根据单元格所在的位置索引获取该单元格中的值，下标从1开始，sheet.cell(row=1,column=1).value,表示Excel中的第一行第一列的值
        if coordinate!=None:
            try:
    ...
    def getCellOfObject(self,sheet,coordinate = None,rowNo = None,colsNo = None):
        #获取某个单元格的对象，可以根据单元格所在位置的数字索引，也可以直接根据Excel中单元格的编码及坐标，如getCellObject(sheet,coordinate = 'A1') or getCellObject(sheet,rowNo = 1,colsNo = 2)
        if coordinate != None:
            try:
    ...
    def writeCell(self,sheet,content,coordinate = None,rowNo = None,colsNo = None,style =None):
        ...
        if coordinate is not None:
            ...
            try:
                ...
                if style:
                    sheet.cell(row = rowNo,column = colsNo).font = Font(self.RGBDict[style])
                self.workbook.save(self.excelFile)
            except Exception as e:
                raise e
        else:
...
if __name__=='__main__':
    pe = ParseExcel()
    pe.loadWorkBook("D:\\Users\\bj\PycharmProjects\DataDrivenFrameWork\\testData\\163邮箱联系人.xlsx")
